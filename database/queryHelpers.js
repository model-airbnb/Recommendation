...
module.exports.getAveragePriceForSearch = (parameters, startDate, endDate) => {
  ...
  const queryText = `SELECT AVG(booked_nights.price::money::numeric::float8) FROM listings INNER JOIN booked_nights ON listings.listing_id = booked_nights.listing_id WHERE ${queryString} AND booked_at >= '${startDate}' AND booked_at <= '${endDate}'`;
